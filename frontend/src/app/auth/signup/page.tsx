...
const SignUp: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || !email || !password || !confirmPassword || !signCode) {
      console.log(
        JSON.stringify({ name, email, password, confirmPassword, signCode }),
      );
    ...
    try {
      const response = await fetch("http://127.0.0.1:5000/api/register", {
        ...
        body: JSON.stringify({ name, email, password, signCode }),
      });

      if (response.status === 401) {
        addAlert("error", "Wrong sign-in code");
        return;
      }
      if (response.status === 409) {
        addAlert("error", "An user with email already exists");
        return;
      }

      addAlert("ok", "Account created successfully");
    } catch (error) {
      ...
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        <div className="flex flex-wrap items-center">
          <div className="hidden w-full xl:block xl:w-1/2">
            <div className="flex items-center px-26 py-17.5 text-center">
              <div>
                <h1 className="mb-6 mt-2 text-4xl font-bold text-black">
                  AviaEase
                </h1>

                <p className="2xl:px-10">
                  A seamless airline database management system for managing
                  everything.
                </p>
